...
def db():
    ...
    c.execute('CREATE TABLE IF NOT EXISTS holdings(ticker TEXT PRIMARY KEY, shares REAL, avg_price REAL)')
    c.commit(); return c
...
def research_prompt(o):
    ...
    return f'''Analyse {o.get("Name","the company")} as an equity research analyst using ONLY the supplied data.\n\n{data}\n\nProduce: business snapshot; financial performance; profitability and balance sheet observations; valuation observations; growth drivers; risks; and questions for further research. Separate facts from interpretation. Do not invent missing data.'''

st.sidebar.title('📈 EquityIQ')
page=st.sidebar.radio('Navigate',['Dashboard','Company Research','Portfolio','Valuation','Report Lab'])
...
if page=='Dashboard':
    ...
    symbol=st.text_input('Ticker / Alpha Vantage symbol','RELIANCE.BSE').strip().upper()
    ...
    if 'o' in st.session_state:
        ...
        cols=st.columns(6)
        vals=[('Market Cap',money(o.get('MarketCapitalization'))),('P/E',o.get('PERatio','—')),('P/B',o.get('PriceToBookRatio','—')),('EV/EBITDA',o.get('EVToEBITDA','—')),('ROE',pct(o.get('ReturnOnEquityTTM'))),('Margin',pct(o.get('ProfitMargin')))]
    ...
    if f:
        if PdfReader is None: st.error('Install pypdf.')
            ...
            st.subheader('AI research prompt'); st.code('''Analyse the supplied annual report. Extract only supported facts. Produce: business model; segment performance; revenue and profitability trends; cash flow and capital allocation; debt/liquidity; management priorities; principal risks; accounting/governance items requiring attention; and five analyst questions. Do not invent data.\n\nREPORT TEXT:\n'''+text[:30000],language='text')
